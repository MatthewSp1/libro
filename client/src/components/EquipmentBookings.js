...
const EquipmentBookings = ({
  ...
  return (
    bookings.length !== 0 && (
      <>
        {bookings.length === 1 ? (
          <h1 className="text-1xl font-bold text-gray-500 mb-2 mt-10">Equipment Booking</h1>
        ) : (
          <h1 className="text-1xl font-bold text-gray-500 mb-2 mt-10">Equipment Bookings</h1>
        )}
        <div className="overflow-x-auto rounded border-2 border-gray-200 shadow w-auto">
          <table className="min-w-full table-fixed bg-white border border-gray-200">
            <thead>
              <tr className="text-left bg-gray-100">
                ...
                <th className="py-2 px-4 border-b">Borrowable From</th>
                <th className="py-2 px-4 border-b">Return Before</th>
            ...
            <tbody>
              {bookings.map((booking) => (
                <tr key={booking._id}>
                  ...
                  <td className="py-3 px-4 border-b">{`${new Date(booking.startDate)
                    .toUTCString()
                    .split(" ")
                    .slice(0, 4)
                    .join(" ")} - ${booking.startTime}`}</td>
                  <td className="py-3 px-4 border-b">{`${new Date(booking.endDate)
                    .toUTCString()
                    .split(" ")
                    .slice(0, 4)
                    .join(" ")} - ${booking.endTime}`}</td>
                  <td className="py-3 px-4 border-b">
