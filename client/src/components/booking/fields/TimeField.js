...
const TimeField = ({
  ...
  const handleStartTimeChange = (time, close) => {
    setSelectedStartTime(time.display);
  ...
  const handleEndTimeChange = (time, close) => {
    setSelectedEndTime(time.display);
    setEndTime(time.value);
    close();
  };

  const filteredEndTimeOptions = isStartTimeSelected
    ? timeOptions.filter(
        (option) =>
  ...
  return (
    <div className="w-full max-w-md">
      <div className="flex flex-col items-center mb-4">
        <label className="block text-gray-700 mb-2">{firstLabel}</label>
                  ...
                  {({ active, close }) => (
                    <button
                      onClick={(e) => {
                        ...
                        handleStartTimeChange(timeOption, close);
                      }}
      ...
      {isStartTimeSelected && (
        <div className="flex flex-col items-center mb-4">
          <label className="block text-gray-700 mb-2">{secondLabel}</label>
          <Menu as="div" className="relative inline-block text-left w-full">
            <div>
              <MenuButton className="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                {selectedEndTime || "-"}
            ...
            >
              <div className="py-1">
                {filteredEndTimeOptions.map((timeOption) => (
                  <MenuItem key={timeOption.value}>
                    {({ active, close }) => (
                      <button
                        onClick={(e) => {
                          e.preventDefault(); // Prevent form submission
                          handleEndTimeChange(timeOption, close);
                        }}
                        className={`block w-full px-4 py-2 text-left text-sm ${
                          active ? "bg-gray-100 text-gray-900" : "text-gray-700"
                        }`}
